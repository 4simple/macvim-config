...
function! SimpleTemplate(tname,tmode)
    "path
    let tpath = exists('g:SimpleTemplate.path') ? g:SimpleTemplate.path : g:SimpleTemplatePath
    ...
    let postfix = exists('mfiletype[1]') ? mfiletype[1] : g:SimpleTemplateFileType
    let tfiletype = postfix == 'js' ? 'javascript' : postfix
    ...
    if a:tmode == 'tab'
        if strlen(tname) && strlen(postfix)
            execute 'tabnew '.tname.'.'.postfix
        elseif strlen(postfix)
    ...
    if hasCursor
        let line = getline('.')
        let repl = substitute(line, g:SimpleTemplateCursorFlag, '', '')
        call setline('.', repl)
    endif
endfunction
